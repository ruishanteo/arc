...
export function CommonRequirements() {
  ...
  function checkIntegratedProj(arr) {
    let commonMod = require("../module_data/integratedproj.json")[
      degrees[0].title
    ];
  ...
  function checkCHSCDE(inputString, arr) {
    ...
    if (inputString === "chs8") {
      const present = arr.filter((moduleCodes) =>
        commonMod.includes(moduleCodes)
      );
      if (present.length >= 2) {
        return "#FFFFFF";
      }
  ...
  function checkPresentCommonMod(inputString) {
    ...
    if (inputString === "cdid") {
      ...
      const hasCommonElement = commonMod.some((element) =>
        arr.includes(element.moduleCode)
      );
      if (hasCommonElement) {
        return color;
      } else {
