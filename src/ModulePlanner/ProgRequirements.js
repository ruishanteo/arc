...
export function ProgRequirements() {
  ...
  function checkcsbreadth() {
    ...
    for (const list of Object.values(data)) {
      const count = arr.filter((module) =>
        list.filter((listModule) => listModule.moduleCode === module)
      ).length;
      if (count >= 3) {
        return "#cff8df";
      }
  ...
  function checkMin(titleArr) {
    ...
    if (titleArr[2].length <= 3) {
      const pres1 = arr.filter((moduleCode) =>
        moduleCode.includes(titleArr[2])
      );
      const recog = titleArr[2] + "-recognised";
      const pres2 = arr.filter((moduleCode) =>
        data[recog].includes(moduleCode)
      );
  ...
  function checkbzaelec() {
    ...
    if (
      pres.length >= 5 &&
      pres.filter((moduleCode) => {
        const match = moduleCode.match(/\d/);
        const firstDigit = parseInt(match[0], 10);
        return firstDigit >= Number(4);
      }).length >= 3 &&
      pres.filter((moduleCode) => moduleCode.includes("BT")).length >= 3
    ) {
      return "#cff8df";
    }
    return "#FFFFFF";
  ...
  function checkcomreq() {
    ...
    let pres = arr.filter(
      (moduleCode) =>
        moduleCode.includes("CS") ||
        moduleCode.includes("IS") ||
        moduleCode.includes("CP")
    );
    if (
      pres.filter((moduleCode) => data.includes(moduleCode)).length >= 1 &&
      pres.filter((moduleCode) => {
        const match = moduleCode.match(/\d/);
        const firstDigit = parseInt(match[0], 10);
  ...
  function checkLSM() {
    ...
    let pres = arr.filter((moduleCode) => data.includes(moduleCode));
    if (
      pres.filter((moduleCode) => moduleCode.includes("LSM22")).length <= 2 &&
      pres.filter(
        (moduleCode) =>
          moduleCode.includes("LSM42") || moduleCode.includes("LSM4991")
      ).length >= 3
    ) {
      const moduleCodesToCheck8 = [
        "LSM4288M",
        "LSM4288C",
        "LSM4288E",
        "LSM4288X",
      ];
      const moduleCodesToCheck4 = [
        "LSM3288",
        "LSM3288R",
        "LSM2288",
        "LSM2288R",
      ...
      if (arr.some((moduleCode) => moduleCodesToCheck8.includes(moduleCode))) {
        const filt = pres.filter(
          (moduleCode) =>
            moduleCode.includes("LSM42") || moduleCode.includes("LSM22")
        );
        const filt2 = pres.filter(
          (moduleCode) =>
            moduleCode.includes("LSM3991") || moduleCode.includes("LSM4991")
        );
        let filt2Length = 0;
        if (filt2.length > 0) {
          filt2Length = 1;
  ...
  function checkBizSpec() {
    ...
    for (const list of Object.values(data)) {
      ...
      if (
        ...
        condition1 = list[0].every((pair) =>
          pair.some((element) => arr.includes(element))
        );
      } else {
  ...
  function checkBizHonours() {
    ...
    if (arr.includes("BHD4001")) {
      if (
        arr.filer(
          (moduleCode) =>
            data[0].includes(moduleCode).length >= 2 || arr.includes("FSP4003")
        )
      ) {
        return "#cff8df";
      }
